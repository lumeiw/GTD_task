            ...
            child: Form(
              child: TaskCardContainer(
                child: Column(
                  ...
                  children: [
                    TaskTitleField(
                      initialValue: state is CreateTaskEditing ? state.title : null,
                      onChanged: (value) => createTaskCubit.updateField(TaskField.title, value),
                    ),
                    TaskNotesField(
                      initialValue: state is CreateTaskEditing ? state.body : null,
                      onChanged: (value) => createTaskCubit.updateField(TaskField.body, value),
                    ),
                    const SizedBox(height: 12),
                    TaskActionBar(task: task, cubit: createTaskCubit, state: state
                    ),
                  ],
                ),
              ),
    ...
    return IconButton(
      ...
      onPressed: () async {
        final selectedDate = await showDatePicker(
          context: context,
          initialDate: (state is CreateTaskEditing && (state as CreateTaskEditing).date != null)
